...
5. Use `hapatlas_schedules_manage` for one Schedule at a time, then use the explicit usage/load/air-input and `hap51-space-dimensions.upsert` actions in `hapatlas_space_inputs_manage` with Schedule identities as read-only dependencies.
...
For direct HAP 5.1 Space dimensions, use `spaces[].legacy_dimensions`: `floor_area` and `average_ceiling_height` are required; `building_weight` is optional. IP means ft², ft, and lb/ft²; SI means m², m, and kg/m². Never manufacture polygon geometry from these values. This path remains Candidate, so dry-run, apply, and inspect all three native values before continuing. HAP 6.3 rejects it and requires real Level/polygon geometry.
