...
Assigned edit, duplicate, delete, placement, quantity, host geometry, opening groups/tags, and assignment remain guarded. HAP 5.1 later assigns one Door type plus quantity to a Space wall exposure. HAP 6.3 graphical placement is engineer-only, and later Space Model assignment may reference an already accepted Door identity. Never route these actions through Window, Wall, Space Inputs, a retired envelope writer, or whole-model apply.
