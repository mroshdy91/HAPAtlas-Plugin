...
| Spaces | Not applicable as a direct manager | Available General, five Internals groups, Usage/OA, Infiltration, four surface-tab activities | Use exactly one advertised Space action per call. Existing Schedules and reusable envelope items are exact identity dependencies, not inline definitions. Require exact native Space ID, immutable dry run, unchanged live request, native readback, untouched-tab/domain verification and serving-System/connected-Plant-only result invalidation. All 13 actions passed their exact-build gates; native input PDFs exported by the engineer are presentation evidence, not automated report capability. |
